    ...
    else // 并行，进程0负责收发和汇总，其他进程负责计算
    {
        ...
        if (rank == 0) // 进程0，主进程
        {
            ...
            for (int i = 0; i < processNum - 1; i++)
            {
                ...
                MPI_Send(&matrixA[beginRow * n + 0], rowsPerProcess * n, MPI_DOUBLE, i + 1, 0, MPI_COMM_WORLD);
            ...
            for (int i = 0; i < processNum - 1; i++)
            {
                beginRow = rowsPerProcess * i;
                endRow = rowsPerProcess * (i + 1);
                MPI_Recv(&matrixC[beginRow * n + 0], rowsPerProcess * n, MPI_DOUBLE, i + 1, 2, MPI_COMM_WORLD, &status);
            }
        ...
        else // 其他进程
        {
            // beginRow = rowsPerProcess * (rank - 1)
            // endRow = rowsPerProcess * rank
            double *matrixA = new double[rowsPerProcess * n];
            double *matrixB = new double[n * n];
            double *matrixC = new double[rowsPerProcess * n];

            MPI_Recv(&matrixA[0 * n + 0], rowsPerProcess * n, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
            MPI_Recv(&matrixB[0 * n + 0], n * n, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, &status);

            matrixMultiply(matrixA, matrixB, matrixC, rowsPerProcess, n);
