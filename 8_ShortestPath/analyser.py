...
for size in grouped["Array_size"].unique():
    base_time = grouped[
        (grouped["Array_size"] == size) & (grouped["Number_of_threads"] == 1)
    ]["Time"].values[0]
...
plt.figure()
sns.lineplot(
    data=grouped, x="Array_size", y="Speedup", hue="Number_of_threads", marker="o"
)
plt.title("Parallel Floyd-Warshall Algorithm Speedup")
