...
for size in grouped['Matrix_size'].unique():
    base_time = grouped[(grouped['Matrix_size'] == size) & (grouped['Number_of_threads'] == 1)]['Time'].values[0]
...
plt.figure()
sns.lineplot(data=grouped, x='Matrix_size', y='Speedup', hue='Number_of_threads', marker='o')
